...
def build_mamba_qr_dqn_command(num_episodes: int, output_dir: Path) -> list[str]:
    return [
        sys.executable,
        "-m",
        "app.mamba2_train_test",
...
def build_mamba_h_dqn_command(num_episodes: int, output_dir: Path) -> list[str]:
    return [
        sys.executable,
        "-m",
        "app.mamba2_train_test",
...
def main() -> None:
    ...
    if "qr_dqn" in selected_models:
        print("Running best Mamba QR DQN hyperparameters...")
        run_command(build_mamba_qr_dqn_command(args.num_episodes, base_out / "qr-dqn"), cwd=root_dir)

    if "h_dqn" in selected_models:
        print("Running best Mamba H DQN hyperparameters...")
        run_command(build_mamba_h_dqn_command(args.num_episodes, base_out / "h-dqn"), cwd=root_dir)
